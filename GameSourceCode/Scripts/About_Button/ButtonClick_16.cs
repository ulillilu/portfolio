...
public class ButtonClick_16 : MonoBehaviour
{
    ...
    {
        if(slots[16].itemID_Text != 0)
        {
            usePanelActive = !usePanelActive;
            usePanel.SetActive(usePanelActive);
        }
        //더 많은 데이터를 불러올수있도록 inventoryslot 단계에서 데이터를 더 추가!
        theDB.selected_Item_ID = slots[16].itemID_Text;
        theDB.selected_Item_Name = slots[16].itemName_Text;
        theDB.selected_Item_Des = slots[16].itemDes_Text;
    }

    void Update()
    {
        if(inven.lenItemList < 17)
        {
            slots[16].RemoveItem();
        }
    }
