...
public class ButtonClick_10 : MonoBehaviour
{
    ...
    {
        if(slots[10].itemID_Text != 0)
        {
            usePanelActive = !usePanelActive;
            usePanel.SetActive(usePanelActive);
        }
        //더 많은 데이터를 불러올수있도록 inventoryslot 단계에서 데이터를 더 추가!
        theDB.selected_Item_ID = slots[10].itemID_Text;
        theDB.selected_Item_Name = slots[10].itemName_Text;
        theDB.selected_Item_Des = slots[10].itemDes_Text;
    }

    void Update()
    {
        if(inven.lenItemList < 11)
        {
            slots[10].RemoveItem();
        }
    }
