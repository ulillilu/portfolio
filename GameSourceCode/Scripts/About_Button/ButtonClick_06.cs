...
public class ButtonClick_06 : MonoBehaviour
{
    ...
    {
        if(slots[6].itemID_Text != 0)
        {
            usePanelActive = !usePanelActive;
            usePanel.SetActive(usePanelActive);
        }
        //더 많은 데이터를 불러올수있도록 inventoryslot 단계에서 데이터를 더 추가!
        theDB.selected_Item_ID = slots[6].itemID_Text;
        theDB.selected_Item_Name = slots[6].itemName_Text;
        theDB.selected_Item_Des = slots[6].itemDes_Text;
    }

    void Update()
    {
        if(inven.lenItemList < 7)
        {
            slots[6].RemoveItem();
        }
    }
