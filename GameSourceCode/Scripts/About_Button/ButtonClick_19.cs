...
public class ButtonClick_19 : MonoBehaviour
{
    ...
    {
        if(slots[19].itemID_Text != 0)
        {
            usePanelActive = !usePanelActive;
            usePanel.SetActive(usePanelActive);
        }
        //더 많은 데이터를 불러올수있도록 inventoryslot 단계에서 데이터를 더 추가!
        theDB.selected_Item_ID = slots[19].itemID_Text;
        theDB.selected_Item_Name = slots[19].itemName_Text;
        theDB.selected_Item_Des = slots[19].itemDes_Text;
    }

    void Update()
    {
        if(inven.lenItemList < 20)
        {
            slots[19].RemoveItem();
        }
    }
